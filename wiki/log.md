...
## 2026-07-18 Figma 목업 기반 HUD 리뉴얼 — 상단바 잘림/기어 죽은버튼/저대비/축 기즈모 어포던스 수정
사용자가 스크린샷(스테이지2 실기기/에디터 캡처) 제보한 4가지 UI 문제를 Figma MCP로 먼저 목업(`Picross 3D UI` 파일, `genmRHW8qAh6Fb3Pv1PQmj`)을 만들어 다듬은 뒤 코드에 반영:
1. **상단바 우측 버튼 잘림(버그)**: `HUD._build()`가 `top_bar` 하나에 순차 배치 + 고정폭 스페이서(70px)로 버튼을 쌓던 방식이라 화면 폭이 좁으면 pause 버튼이 화면 밖으로 잘려나감. `left_group`(뱃지+home, TOP_LEFT 앵커) / `center_group`(하트, TOP_WIDE+alignment=CENTER — title_col/bottom_row와 동일 패턴 재사용) / `right_group`(reset+pause, TOP_RIGHT 앵커+`grow_horizontal=GROW_DIRECTION_BEGIN`) 3그룹으로 완전히 분리 — 화면 폭에 무관하게 항상 안전.
2. **죽은 기어(⚙) 버튼**: 시그널 미연결 상태로 방치돼 눌러도 반응 없던 버튼. 사용자 확인 후 제거(나중에 설정 기능 붙일 계획 생기면 그때 자리 다시 추가하기로).
3. **저대비 텍스트**: 미스터리 타이틀/타이머/회전힌트 전부 쓰던 `INK_SOFT`(#6d8c9d, 흰 배경 대비 ~3:1, WCAG AA 미달)를 `INK`(#42606f, ~7:1)로 교체하고 `HUD.gd`에서 `INK_SOFT` 상수 자체를 삭제(다른 곳에서 안 씀, `Home.gd`는 별도 로컬 상수라 무관). pill 배경도 반투명 0.7~0.72 → 불투명 0.92+1px 테두리(`CARD_SHADOW`)로 바꿔 민트 배경과 경계가 뚜렷해짐. `show_status()`(CLEAR!/GAME OVER)에도 아웃라인 추가 — 3D 씬 위에 배경 없이 얹히는 텍스트라 파스텔 하늘색과 섞이는 문제 방지.
4. **버튼 위계 없음**: 6개 아이콘 버튼(뱃지+home+하트+reset+gear+pause)이 전부 동일한 흰카드 스타일이던 것을, `_make_primary_icon_button()`(INK 배경 채움, 자주 쓰는 pause 전용) 신설로 주/보조 구분. `set_reset_enabled(false)` 상태의 시각 피드백도 `font_disabled_color`+`disabled` 스타일박스(불투명도 0.55)로 명확화(이전엔 커스텀 disabled 스타일 자체가 없어서 Godot 기본값에 의존했음). 스테이지 뱃지도 감싸던 흰 카드+"스테이지" 텍스트 라벨 제거하고 숫자 칩만 남김(폭 절약 + 중복 정보 제거).
5. **축 기즈모(원뿔) 어포던스 없음**: `AxisGizmo._add_arrow()`에 축 문자(X/Y/Z) `Label3D`를 빌보드로 붙여봤으나(`billboard=BILLBOARD_ENABLED`, `no_depth_test=true`) 사용자가 에디터 확인 후 "글씨 지워달라"고 피드백 — 원뿔 색상만으로 충분하다고 판단, 롤백. 색깔 원뿔 자체는 그대로 유지, 라벨만 제거. 어포던스 개선은 이번엔 보류.

별개로 같은 QA 스크린샷에서 **클루 라벨 좌우反전 버그**도 발견: `ClueDisplay._face_rotation()`이 X/Z축 측면 라벨 회전을 오일러각(`Vector3(0, PI/2*sign, 0)` 등) 수식으로 손으로 유도했는데, 특정 부호(-1)에서 라벨 뒷면이 카메라를 향해 `Label3D.double_sided` 특성상 글자가 거울에 비친 것처럼 반전돼 보이는 것으로 추정. `_apply_face_orientation()`으로 교체 — X/Z 측면은 `Basis.looking_at(-normal, Vector3.UP)`으로 바깥쪽 법선을 향해 견고하게 정렬(기존 "Z+ 기본(무회전)" 케이스가 이 공식과 정확히 일치함을 확인해 회귀 없음 검증), Y(상/하)면은 법선이 world up과 평행해 `looking_at`의 up 기준이 성립하지 않으므로 기존 X축 회전 방식 그대로 유지.

헤드리스 검증: `tests/test_puzzle_model.gd`(23 passed) / `tests/test_axis_gizmo.gd`(12 passed) 회귀 전부 통과, `godot --headless --quit-after 60 Main.tscn` 파싱/런타임 에러 없이 클린 종료(exit 0) 확인. 사용자가 에디터로 직접 확인해 위 5개 항목 + 클루 라벨 반전 수정 전부 승인, 기즈모 라벨만 롤백 (→ [[architecture]], [[core-decisions]]).
